...
- (id)initWithTitle:(NSString *)aTitle options:(NSArray *)aOptions xy:(CGPoint)point size:(CGSize)size isMultiple:(BOOL)isMultiple id:(NSString *)id
{
    ...
    if (self = [super initWithFrame:rect])
    {
        ...
        if (isMultipleSelection) {
            ...
            [btnDone setFrame:CGRectMake(rect.origin.x+200,rect.origin.y, 82, 31)];
...
-(void)Click_Done{
    
    {
        ...
        for (int k=0; k<self.arryData.count; k++) {
            ...
            [arryResponceData addObject:[NSNumber numberWithInt:path.row]];
            NSLog(@"pathRow=%d",path.row);
        }
    
        [self.delegate didSelectIndices:arryResponceData withId:id inOptions:_kDropDownOption];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
   ...
   //     if (self.delegate && [self.delegate respondsToSelector:@selector(DropDownListView:didSelectedIndex:)]) {
        [self.delegate didSelectIndex:[indexPath row] withId:id inOptions:_kDropDownOption];
